...
from model.schema import BodyCollectEvent, Order, SearchedEvent, SearchResponse
...
def send_to_sqs(event: BodyCollectEvent) -> bool:
    try:
        ...
        # add time - %Y-%m-%dT%H:%M:%S.%3f
        event_datetime = datetime.datetime.utcnow().isoformat(timespec='milliseconds')
        event_dict.update({"event_datetime": event_datetime})

        event_obj = json.dumps(event_dict)
...
def collected_json_to_schema(item: dict) -> Tuple[BodyCollectEvent, Union[Order, None]]:
    order = None
    parameters = item.pop("parameters", None)
    event = BodyCollectEvent(**item)
